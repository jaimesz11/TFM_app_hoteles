...
def app():
    ...
    with c2:    
        ...
        if carga is not None:
            ...
            df["review"] = df['review'].apply(lambda x: decontracted(x)) # Quitar las contracciones
            df["sentimiento"] = df['review'].apply(lambda x: analizador.polarity_scores(x)) # Extracción de sentimiento
            df = pd.concat([df.drop(['sentimiento'], axis=1), df['sentimiento'].apply(pd.Series)], axis=1)
            
            # Con TextBlob
            df['polaridad'] = df['review'].apply(lambda x: TextBlob(x).sentiment.polarity) # polaridad
            df['subjetividad'] = df['review'].apply(lambda x: TextBlob(x).sentiment.subjectivity) # subjetividad
